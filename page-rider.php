...
.rider-loading{text-align:center;padding:120px 0;color:rgba(255,255,255,.5);font-size:14px}
...
.rider-meta{display:flex;gap:14px;flex-wrap:wrap;margin-top:8px;font-size:13px;color:rgba(255,255,255,.5)}
...
.rider-empty{color:rgba(255,255,255,.45);font-size:13px;padding:20px 0}
...
.rider-not-found p{color:rgba(255,255,255,.5);font-size:14px}
</style>
...
<div class="rider-wrap">
  ...
  <div id="riderContent" style="display:none">
    <div class="rider-head">
      ...
      <div>
        <h1 class="rider-name" id="riderName" style="margin:0">—</h1>
    ...
    <h2 class="rider-section-title">Road Stories <span id="riderBlogCount"></span></h2>
...
(function() {
  ...
  function render(d) {
    ...
    avWrap.innerHTML = p.avatar_url
      ? '<img class="rider-avatar" src="' + p.avatar_url + '" alt="' + firstName + ' profile photo">'
    ...
    if (!albums.length) {
      ...
      albEl.innerHTML = albums.map(function(a) {
        ...
        var photoHtml = photos.map(function(p){ return '<img src="' + p.url + '" alt="' + (a.title||'Trip photo') + '" loading="lazy">'; }).join('');
    ...
    if (!blogs.length) {
      ...
      blogEl.innerHTML = blogs.map(function(b) {
        var cover = b.cover_image ? '<img class="rider-blog-cover" src="' + b.cover_image + '" alt="' + b.title + ' cover" loading="lazy">' : '<div class="rider-blog-cover"></div>';
        return '<div class="rider-blog-row">' + cover + '<div><div style="color:#fff;font-size:14px">' + b.title + '</div><div style="color:rgba(255,255,255,.5);font-size:11px;margin-top:3px">' + new Date(b.created_at).toLocaleDateString('en-IN',{day:'numeric',month:'short',year:'numeric'}) + '</div></div></div>';
      }).join('');
    }
